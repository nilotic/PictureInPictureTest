...
struct DampedHarmonicSpring {
    ...
    private var dampedNaturalFrequency: CGFloat {
        return undampedNaturalFrequency * sqrt(abs(1.0 - pow(dampingRatio, 2.0)))
    }
    ...
    init(dampingRatio: CGFloat, frequencyResponse: CGFloat) {
        ...
        mass               = 1.0
        stiffness          = pow(2.0 * .pi / frequencyResponse, 2.0) * mass
        dampingCoefficient = 4.0 * .pi * dampingRatio * mass / frequencyResponse
    }
    ...
    func position(at time: TimeInterval, initialPosition: CGFloat = 1.0, initialVelocity: CGFloat = 0) -> CGFloat {
        let ζ   = dampingRatio
        let λ   = dampingCoefficient / mass / 2.0
    ...
    private func maximumDisplacementFromEquilibrium(initialVelocity: CGFloat) -> CGFloat {
        ...
        var t: TimeInterval {
            if abs(ζ - 1.0) < 1e-6 {
                ...
                return TimeInterval(log((λ + ω_d) / (λ - ω_d)) / (2.0 * ω_d))
            }
        }
    ...
    func timingFunction(withInitialVelocity initialVelocity: CGFloat, from currentValue: inout CGFloat, to targetValue: CGFloat, context: UITraitEnvironment) -> UISpringTimingParameters {
        ...
        let epsilon = 1.0 / fmax(1.0, context.traitCollection.displayScale)
        return timingFunction(withRelativeInitialVelocity: relativeVelocity(forVelocity: initialVelocity, from: &currentValue, to: targetValue, epsilon: epsilon))
    }
    ...
    /// Transforms the specified velocity to a relative velocity with a value of `1.0` indicating that the distance from the specified current value to the specified target value is covered in one second.
    /// If the current and target values match already, the current value is slightly displaced so that the initial velocity can be respected.
    ///
