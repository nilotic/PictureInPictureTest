...
public struct DampedHarmonicSpring {
    ...
    /// per second.
    public func position(at time: TimeInterval, initialPosition: CGFloat = 1.0, initialVelocity: CGFloat = 0) -> CGFloat {
        let ζ   = dampingRatio
    ...
    /// - parameter context: The context in which the animation takes place,
    /// used to align values on pixel boundaries.
    public func timingFunction(withInitialVelocity initialVelocity: CGPoint, from currentValue: inout CGPoint, to targetValue: CGPoint, context: UITraitEnvironment) -> UISpringTimingParameters {
